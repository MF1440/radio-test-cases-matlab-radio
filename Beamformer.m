classdef Beamformer < handle
    ...
    properties (Access = private)
        horizontalElementCount
        verticalElementCount
        allocationMatrix
        multiuserLayout
        userCount
        snrdB
    ...
    methods
        ...
        function this = Beamformer(simulationParams)
            ...
            this.horizontalElementCount =   simulationParams.horizontalElementsCount;
            this.verticalElementCount =     simulationParams.verticalElementsCount;
            this.userCount =                simulationParams.nUsers;
        ...
        function calcChannelRealization(this)
            ...
            elementSpacing = 0.5;
            % Количество панелей
            verticalPanelCount = 1;
            horizontalPanelCount = 1;
            % Межпанельное расстояние
            vertPanSpacing = 0;
            horizPanSpacing = 0;
            % Передающая антенна КА
            satTransmitAnt = qd_arrayant('3gpp-nr', this.horizontalElementCount,...
                this.verticalElementCount,...
                s.center_frequency, polInd, 0, elementSpacing, verticalPanelCount,...
                horizontalPanelCount, vertPanSpacing, horizPanSpacing);
            ...
            this.channelCoeffArray = rawChannelCoeffs ./ sqrt(averacalcransmissionGain);
        ...
        function calcBeamformerWeights(this)
            ...
            nTransmitAntennas = size(this.channelCoeffArray, 2);
            % Если все передающие антенны могут работать со всеми пользователями то
            if isempty(this.allocationMatrix)
                D = repmat(eye(nTransmitAntennas), [ 1 1 this.userCount ]);
            end
            % Предопределим матрицу под весовые коэффициенты
            this.beamformerWeightArray = zeros(size(this.channelCoeffArray'));
            switch this.beamformerMethod
                case 'MRT'
                    % Расссчитаем весовые коэффициенты матрицы
                    for userIdx = 1 : this.userCount
                        % коэффициенты канала c учетом тензора D
                        channelVector = (this.channelCoeffArray(userIdx, :) * D(:, :, userIdx))';
                        % нормируем результат
                        this.beamformerWeightArray(:, userIdx) = channelVector / norm(channelVector);
                    end
                case 'ZF'
                    % Расссчитаем весовые коэффициенты матрицы
                    for userIdx = 1 : this.userCount
                        % коэффициенты канала c учетом тензора D
                        effectiveChannel = (this.channelCoeffArray * D(:, :, userIdx))';
                        ...
                        this.beamformerWeightArray(:,userIdx) = channelInversion(:, userIdx) / norm(channelInversion(:, userIdx));
                    end
                otherwise
        ...
        function calcSpectralPerformance(this)
            ...
            this.spectralPerformance = sum(log2(1 + signalGainArray ./ (db2pow(- this.snrdB) + interferenceGains)), 1);
